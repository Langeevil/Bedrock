import { STATUS_NODE_COLOR } from "../constants";

export function buildGraph(tasks, tags, W, H) {
  const cx = W / 2;
  const cy = H / 2;
  ...
  const nodes = [
    ...
    ...tasks.map((tk, i) => {
      ...
      return {
        ...
        label: tk.title,
        type: "task",
        color: STATUS_NODE_COLOR[tk.status],
        r: 15,
        taskId: tk.id,
        status: tk.status,
  ...
  const edges = [];
  tags.forEach((tg) => edges.push({ s: "p-p1", t: `tg-${tg.id}`, rel: "has_tag" }));
  tasks.forEach((tk) => {
    edges.push({ s: "p-p1", t: `tk-${tk.id}`, rel: "contains" });
...
export function drawGraph(ctx, W, H, nodes, edges, cam) {
